...
$sql = "SELECT nombre, apellido FROM profesor";
$resultado = $conn->query($sql);

$sql2 = "SELECT nombre_materia FROM materia limit 1";
$materia = $conn->query($sql2);

$rowCount = $resultado ? $resultado->rowCount() : 0;
$mat = $materia ? $materia->rowCount() : 0;
?>
...
<body>
    ...
    <form method="post" action="pdfgen2.php">
        <table>
            <tr>
                <td colspan="2">DOCENTE(S):</td>
                <td colspan="3">
                <input type="hidden" name="docentes" value="<?php if ($rowCount > 0) {
    ...
    if ($mat > 0) {
        while ($row = $materia->fetch(PDO::FETCH_ASSOC)) {
            echo htmlspecialchars($row["nombre_materia"]);
        }
    } else {
